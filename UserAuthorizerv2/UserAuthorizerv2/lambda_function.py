...
region = 'us-east-2'
userpoolId = 'us-east-2_HV4giMq6j'
appClientId = '2o8j3tsgotv2e4k7q0ibdt4t0m' 
keysUrl = 'https://cognito-idp.{}.amazonaws.com/{}/.well-known/jwks.json'.format(region, userpoolId)


def getValueIfNotNull(event,key):
    value = ""
    try:
      value = event[key]
    except Exception:
...
def lambda_handler(event, context):
    ...
    serviceid = getValueIfNotNull(event,'jobtypeid')
    ...
    # This info will be used to validate
    # TODO: get signed tokens and verify the signature
    # For now we will verify the user manually by checking
    # our tables
    ...
    ## temporary logic. to be removed
    authorizationCode = txns.authorizeUser(orgid,phonenumber)
    if authorizationCode != 200:
        print("Deny all methods")
        policy.denyAllMethods()
        
    else:
        print("Allow all methods")
